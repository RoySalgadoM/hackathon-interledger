...
export class ResponseService {
  ...
  /**
   * Generate gateway timeout response (504)
   */
  generateResponseGatewayTimeout(
    request: FastifyRequest,
    title: string = 'Gateway Timeout',
    description: string = RESPONSE_MESSAGES.GATEWAY_TIMEOUT
  ): ApiResponse {
    const requestId = this.getRequestId(request);
    ...
    return {
      ...
      code: RESPONSE_CODES.GATEWAY_TIMEOUT,
      request_id: requestId,
      data: {}
    };
  }
